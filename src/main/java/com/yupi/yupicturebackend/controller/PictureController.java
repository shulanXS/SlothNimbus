...
//    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    ...
    /**
     * 处理图片删除请求的控制器方法
     * 该方法使用POST请求映射到"/delete"路径，用于删除图片资源
     * 需要检查用户是否具有删除图片的权限
     *
     * @param deleteRequest 包含要删除图片ID的请求体如果请求体为空或ID无效，将抛出业务异常
     * @param request       HTTP请求对象，用于获取当前登录用户信息
     * @return 返回一个表示操作成功的布尔值封装在BaseResponse中
     * @throws BusinessException 如果参数无效或删除操作不符合业务规则，抛出此异常
     */
    @PostMapping("/delete")
    @SaSpaceCheckPermission(value = SpaceUserPermissionConstant.PICTURE_DELETE)
    public BaseResponse<Boolean> deletePicture(@RequestBody DeleteRequest deleteRequest
            , HttpServletRequest request) {
        // 检查删除请求是否有效，如果请求或请求中的ID无效，抛出业务异常
        if (deleteRequest == null || deleteRequest.getId() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        // 获取当前登录用户，用于执行删除操作的权限验证和审计
        User loginUser = userService.getLoginUser(request);
        // 调用服务层方法实际执行图片删除操作
...
//            }
     ...
     * 以图搜图
     *
     * 该接口允许用户通过提供图片ID来搜索相似的图片
     * 主要用于图片检索功能，找到与给定图片相似的其他图片
     *
     * @param searchPictureByPictureRequest 包含图片ID的请求对象，用于指定搜索的图片
     * @return 返回一个包含搜索结果的BaseResponse对象，搜索结果是ImageSearchResult的列表
     */
    ...
    public BaseResponse<List<ImageSearchResult>> searchPictureByPicture(@RequestBody SearchPictureByPictureRequest searchPictureByPictureRequest) {
        ...
        // 检查图片ID是否有效，如果无效则抛出参数错误异常
        ThrowUtils.throwIf(pictureId == null || pictureId <= 0, ErrorCode.PARAMS_ERROR);

        // 根据图片ID获取图片对象
        Picture picture = pictureService.getById(pictureId);

        // 检查图片对象是否为空，如果为空则抛出未找到错误异常
        ThrowUtils.throwIf(picture == null, ErrorCode.NOT_FOUND_ERROR);

        // 调用图片搜索API，根据图片URL搜索相似图片，获取搜索结果列表
